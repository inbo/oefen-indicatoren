...
sti <- read.csv("source/klimaatindicator_planten/data/SpeciesSTI.csv", sep = ";", dec = ",") # STI Sparrius et al.
str(sti)
...
sti_gbif <- gbif_species_name_match(df = sti, name = "speciesname") %>%
  filter(!is.na(scientificName)) %>%
  select(scientificName, speciesname, temp = sti, sd)
...
flora_sti <- inner_join(x = sti_gbif, y = flora_gbif, by = "scientificName") %>%
  mutate(temp_se = sd) # hoe bereken je SE uit sd zonder n te kennen?
str(flora_sti)

# species in temperature dataset and yeareffect dataset must be the same
flora_sti_effects <- flora_sti %>%
...
str(flora_sti_effects)

yeareffects_1950_sti <- yeareffects_1950 %>%
  filter(NaamWetenschappelijk %in% flora_sti$NaamWetenschappelijk) %>%
  mutate(NaamWetenschappelijk = as.factor(NaamWetenschappelijk)) %>%
  arrange(NaamWetenschappelijk)
str(yeareffects_1950_sti)
...
CTI_1950_rel <- calc_cti(
  traitdata = flora_sti_effects, # dataframe met NaamWetenschappelijk, temp en temp_se (1 rij per soort)
  yeareffectdata = yeareffects_1950_sti, # dataframe met de modelschattingen van de soorten
...
flora_sti_effects_hab <- flora_sti_effects %>%
  inner_join(spp_runhaar, by = "NaamWetenschappelijk")
yeareffects_1950_sti_hab <- yeareffects_1950_sti %>%
  inner_join(spp_runhaar, by = "NaamWetenschappelijk")

flora_sti_effects_bos <- flora_sti_effects_hab %>%
  filter(habitatgroep == "bos")
flora_sti_effects_urb <- flora_sti_effects_hab %>%
  filter(habitatgroep == "urbaan")
flora_sti_effects_gras <- flora_sti_effects_hab %>%
  filter(habitatgroep == "grasland")
flora_sti_effects_moeras <- flora_sti_effects_hab %>%
  filter(habitatgroep == "moeras")
flora_sti_effects_akker <- flora_sti_effects_hab %>%
  filter(habitatgroep == "akker")
flora_sti_effects_heide <- flora_sti_effects_hab %>%
  filter(habitatgroep == "heide")
flora_sti_effects_zout <- flora_sti_effects_hab %>%
  filter(habitatgroep == "brakke tot zilte milieus")
flora_sti_effects_duin <- flora_sti_effects_hab %>%
  filter(habitatgroep == "duinen")
flora_sti_effects_zoet <- flora_sti_effects_hab %>%
  filter(habitatgroep == "zoet water")

yeareffects_1950_sti_bos <- yeareffects_1950_sti_hab %>%
  filter(habitatgroep == "bos")
yeareffects_1950_sti_urb <- yeareffects_1950_sti_hab %>%
  filter(habitatgroep == "urbaan")
yeareffects_1950_sti_gras <- yeareffects_1950_sti_hab %>%
  filter(habitatgroep == "grasland")
yeareffects_1950_sti_moeras <- yeareffects_1950_sti_hab %>%
  filter(habitatgroep == "moeras")
yeareffects_1950_sti_akker <- yeareffects_1950_sti_hab %>%
  filter(habitatgroep == "akker")
yeareffects_1950_sti_heide <- yeareffects_1950_sti_hab %>%
  filter(habitatgroep == "heide")
yeareffects_1950_sti_zout <- yeareffects_1950_sti_hab %>%
  filter(habitatgroep == "brakke tot zilte milieus")
yeareffects_1950_sti_duin <- yeareffects_1950_sti_hab %>%
  filter(habitatgroep == "duinen")
yeareffects_1950_sti_zoet <- yeareffects_1950_sti_hab %>%
  filter(habitatgroep == "zoet water")

CTI_1950_rel_bos <- calc_cti(
  traitdata = flora_sti_effects_bos,
  yeareffectdata = yeareffects_1950_sti_bos,
  nrep = 1000,
  interval = 0.95,
  relatief = TRUE) %>%
...
CTI_1950_rel_urb <- calc_cti(
  traitdata = flora_sti_effects_urb,
  yeareffectdata = yeareffects_1950_sti_urb,
  nrep = 1000,
  interval = 0.95,
  relatief = TRUE) %>%
...
CTI_1950_rel_gras <- calc_cti(
  traitdata = flora_sti_effects_gras,
  yeareffectdata = yeareffects_1950_sti_gras,
  nrep = 1000,
  interval = 0.95,
  relatief = TRUE) %>%
...
CTI_1950_rel_moeras <- calc_cti(
  traitdata = flora_sti_effects_moeras,
  yeareffectdata = yeareffects_1950_sti_moeras,
  nrep = 1000,
  interval = 0.95,
  relatief = TRUE) %>%
...
CTI_1950_rel_akker <- calc_cti(
  traitdata = flora_sti_effects_akker,
  yeareffectdata = yeareffects_1950_sti_akker,
  nrep = 1000,
  interval = 0.95,
  relatief = TRUE) %>%
...
CTI_1950_rel_heide <- calc_cti(
  traitdata = flora_sti_effects_heide,
  yeareffectdata = yeareffects_1950_sti_heide,
  nrep = 1000,
  interval = 0.95,
  relatief = TRUE) %>%
...
CTI_1950_rel_zout <- calc_cti(
  traitdata = flora_sti_effects_zout,
  yeareffectdata = yeareffects_1950_sti_zout,
  nrep = 1000,
  interval = 0.95,
  relatief = TRUE) %>%
...
CTI_1950_rel_duin <- calc_cti(
  traitdata = flora_sti_effects_duin,
  yeareffectdata = yeareffects_1950_sti_duin,
  nrep = 1000,
  interval = 0.95,
  relatief = TRUE) %>%
...
CTI_1950_rel_zoet <- calc_cti(
  traitdata = flora_sti_effects_zoet,
  yeareffectdata = yeareffects_1950_sti_zoet,
  nrep = 1000,
  interval = 0.95,
  relatief = TRUE) %>%
